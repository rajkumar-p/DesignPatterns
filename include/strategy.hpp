...
public:
    ...
    void clean_files(const std::string &root, bool recursive);
};

class FilesPurger {
public:
    virtual ~FilesPurger();
    virtual void purge(const std::string &root, bool recursive) = 0;
};
...
public:
    ...
    void purge(const std::string &root, bool recursive) override;
};
